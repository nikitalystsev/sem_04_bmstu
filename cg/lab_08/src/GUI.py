...
class MyWindow(tk.Tk):
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self.title("Лабораторная №8, Лысцев Никита ИУ7-43Б")
        ...
        # виджеты для добавления отрезка
        # -----------------------------------------------
        self._lbl_add_line = self.__create_label("Добавление отрезка")
        self._lbl_add_line.config(font=(FONT, 18, 'bold', "underline"))
        self._lbl_add_line.grid(row=0, column=0, columnspan=4, sticky='wens')

        self._lbl_add_x_start = self.__create_label("X_start:")
        self._lbl_add_x_start.grid(row=1, column=0, sticky='wens')

        self._entry_add_x_start = self.__create_entry()
        self._entry_add_x_start.grid(row=1, column=1, sticky='wens')

        self._lbl_add_y_start = self.__create_label("Y_start:")
        self._lbl_add_y_start.grid(row=1, column=2, sticky='wens')

        self._entry_add_y_start = self.__create_entry()
        self._entry_add_y_start.grid(row=1, column=3, sticky='wens')

        self._lbl_add_x_end = self.__create_label("X_end:")
        self._lbl_add_x_end.grid(row=2, column=0, sticky='wens')

        self._entry_add_x_end = self.__create_entry()
        self._entry_add_x_end.grid(row=2, column=1, sticky='wens')

        self._lbl_add_y_end = self.__create_label("Y_end:")
        self._lbl_add_y_end.grid(row=2, column=2, sticky='wens')

        self._entry_add_y_end = self.__create_entry()
        self._entry_add_y_end.grid(row=2, column=3, sticky='wens')

        self._btn_add_line = self.__create_button("Добавить отрезок")
        self._btn_add_line.config(command=self.__add_line, font=(FONT, 16))
        self._btn_add_line.grid(row=3, column=0, columnspan=4, sticky='wens')
        # -----------------------------------------------
        self._btn_build_parallel = self.__create_button("Построить параллельные отрезки")
        self._btn_build_parallel.config(command=self.__add_parallel_lines, font=(FONT, 16))
        self._btn_build_parallel.grid(row=4, column=0, columnspan=4, sticky='wens')
        ...
        self._lbl_add_cutter.grid(row=5, column=0, columnspan=4, sticky='wens')

        self._lbl_add_x = self.__create_label("X: ")
        self._lbl_add_x.grid(row=6, column=0, sticky='wens')

        self._entry_add_x = self.__create_entry()
        self._entry_add_x.grid(row=6, column=1, sticky='wens')

        self._lbl_add_y = self.__create_label("Y: ")
        self._lbl_add_y.grid(row=6, column=2, sticky='wens')

        self._entry_add_y = self.__create_entry()
        self._entry_add_y.grid(row=6, column=3, sticky='wens')

        self._btn_add_vertex = self.__create_button("Добавить вершину")
        self._btn_add_vertex.config(command=self.__add_point, font=(FONT, 16))
        ...
        self._btn_close_cutter.config(command=self.__close, font=(FONT, 16))
    ...
    def __add_point(self) -> None:
        ...
        self._plane.create_point(x, y, color=BLACK)
    ...
    def __add_parallel_lines(self):
        """
        Метод позволяет добавить отрезков, параллельных сторонам отсекателя
        """
        self._plane.add_parallel_lines()
