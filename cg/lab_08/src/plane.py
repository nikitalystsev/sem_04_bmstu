...
from CyrusBeck import CurysBeck
from dda import Dda
...
class PlaneCanvas(tk.Canvas):
    ...
    def __init__(self, color_cutter: str, color_line: str, color_result: str, master=None, **kwargs):
        ...
        self._list_cutter: list[Figure] = [Figure()]  # массив отсекателей
        ...
        self.bind("<Button-1>", self.get_line_by_click)  # левая кнопка мыши
    ...
    @staticmethod
    def __find_parallel(k: int | float, x1: int | float, y1: int | float, x2: int | float):
        """
        Функция ищет уравнение прямой, параллельной переданному отрезку
        """
        return y1 + k * (x2 - x1)

    def add_parallel_lines(self):
        """
        Метод позволяет добавить несколько линий, параллельных сторонам отсекателя
        """
        cutter = self._list_cutter[0]

        if len(cutter) == 0:
            messagebox.showwarning("", "Отсутствует отсекатель!")
            return
        ...
        for i in range(len(cutter) - 1):
            x1, y1 = cutter[i].x, cutter[i].y
            x2, y2 = cutter[i + 1].x, cutter[i + 1].y
            ...
            if dx > dy:
                line1 = Line(
                    Point(x=x1, y=(y1 + 0.5 * dy)),
                    Point(x=x2, y=self.__find_parallel(k, x1, y1 + 0.5 * dy, x2))
                )
                line2 = Line(
                    Point(x=x1, y=(y1 - 0.5 * dy)),
                    Point(x=x2, y=self.__find_parallel(k, x1, y1 - 0.5 * dy, x2))
                ...
                self._list_line.extend([line1, line2])

                self.draw_line_on_img(line1.point_start, line1.point_end, self.color_line)
                self.draw_line_on_img(line2.point_start, line2.point_end, self.color_line)
    ...
    def cut(self):
        ...
        cutter_alg = CurysBeck(self, self._img, self._list_line, self._list_cutter, self._color_result)
