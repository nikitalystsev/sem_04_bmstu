...
error_t read_point(point_t *const point, FILE *file)
{
    ...
    if (!point || !file)
        rc = ERR_WRONG_PARAMS;
    else if(fscanf_s(file, "%lf %lf %lf", &point->x, &point->y, &point->z) != 3)
        rc = ERR_READ_FILE;
...
static error_t rotate_around_x(point_t *const point, const double angle)
{
    error_t rc = SUCCESS;
...
static error_t rotate_around_y(point_t *const point, const double angle)
{
    error_t rc = SUCCESS;
...
static error_t rotate_around_z(point_t *const point, const double angle)
{
    error_t rc = SUCCESS;
