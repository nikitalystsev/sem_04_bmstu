...
class SplineThreeVar:
    """
    Класс для интерполяции сплайном функции от 3-х переменных
    """
    ...
    # пусть трехмерная интерполяция по x, y
    ...
    def interp_three_var(self) -> int | float:
        ...
        for i in range(len(self.z_s)):
            spline = SplineTwoVar(self.x, self.y, self.x_s, self.y_s, self.values[i])
        ...
        spline = SplineOneVar(self.z, data_table)
