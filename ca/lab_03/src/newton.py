...
class NewtonThreeVar:
    """
    Класс для интерполяции полиномом ньютона функции от 3-х переменных
    """
    ...
    # пусть двумерная интерполяция по x, y

    def get_table_value_for_z(self) -> int:
        """
        Функция ищет ближайшее к z табличное значение
        """
        diff = m.fabs(self.z - self.z_s[0])

        index = 0

        for ind, z in enumerate(self.z_s):
            if m.fabs(self.z - z) < diff:
                diff = m.fabs(self.z - z)
                index = ind
    ...
    def collect_config(self) -> list[int]:
        """
        Функция собирает конфигурацию по z
        """
        index = self.get_table_value_for_z()
        ...
        for i in range(self.nz):
            if i % 2 == 0:
                ...
                if right == len(self.z_s) - 1:
                    left -= 1
                else:
                    right += 1

        print(f"Конфигурация по Z:", list(range(left, right + 1)))

        return list(range(left, right + 1))

    def interp_three_var(self) -> int | float:
        """
        Интерполяция по трем переменным
        """
        config_points = self.collect_config()
        ...
        for i in config_points:
            newton = NewtonTwoVar(self.x, self.y, self.x_s, self.y_s, self.values[i], self.nx, self.ny)
            curr_u = newton.interp_two_var()
            u_s_interp.append(curr_u)

        data_table = list()

        print(f"u_s_interp = {u_s_interp}")

        for i, ind in enumerate(config_points):
            data_table.append((self.z_s[ind], u_s_interp[i]))

        print(f"data_table for 3 = {data_table}")
